...
from src.selenium_operations.website_actions import (navigate_to_url, select_tariff_program, enter_company_name, click_find_tariff, click_oil_tariff_program, click_actual_tariff_option, find_last_record_in_table, click_effective_file_option, get_company_name_from_results, get_tariff_program_from_results, get_oil_tariff_program_from_results, switch_to_iframe, click_download_file_option, click_close)
...
if __name__ == "__main__":
    ...
    if pipelines is not None:
        for pipeline in pipelines['PipelineName']:
            ...
            pipeline_folder = create_pipeline_folder(mainpath=base_path, path="input_files", pipeline_name=pipeline.strip()) 
            start = dt.now()
            ...
            try:
                ...
                if tariff_bill is not None:
                    ...
                    click_download_file_option(driver, download_file_option)
                    file_downloaded = True
                    time.sleep(5)  # Wait for the file to download
                    click_close(driver, close)
